...
module.exports = {
	Mutation: {
		login: async (_, { email, password }, context) => {
			...
			if (!user) return new Error(invalidLoginMessage);

			if (!user.status) return new Error('Your account is not active');

			const isValidPassword = context.app.bcrypt.compare(
				password,
				user.password
			);

			if (!isValidPassword) return new Error(invalidLoginMessage);

			const token = await context.app.crypto.token();
			const userToken = await context.app.crypto.encrypt(

...
module.exports = {
	Query: {
		...
		user: async (_, { id }, context) => {
			try {
				const user = await context.app.knex('users').where({ id }).first();
	...
	Mutation: {
		...
		follow: async (_, args, { app, current_user }) => {
			if (!current_user) throw new Error('Only logged in user can follow');
			...
			let followerId = null;
			if (current_user.role === 'admin') followerId = args.followerId;

			if (!followerId) followerId = current_user.id;
			...
			const record = await knex('users_followers')
				.where({ user_id: userId, follower_id: followerId })
				.first();
			if (!record)
				await knex('users_followers').insert({
					user_id: userId,
		...
		unfollow: async (_, args, { app, current_user }) => {
			if (!current_user) throw new Error('Only logged in user can unfollow');

...
module.exports = {
  Query: {
    ...
    cats: (_, { pageIndex, pageSize = 10 }, { app, current_user }) => {
      ...
      return helpers.getWithPagination(app.knex, { tableName: 'cats', conditions: filter, options: { pageIndex, pageSize } });
    },

    cat: async (_, { id }, context) => {
      const { current_user } = context;

      const cat = await context.app.knex('cats')
        .where({ id })
        .first();
    ...
    catsByUserId: (_, { userId, pageIndex, pageSize = 10 }, { app, current_user }) => {
      // console.log('pageIndex', pageIndex);
      // console.log('pageSize', pageSize);
      ...
      return helpers.getWithPagination(app.knex, {
        tableName: 'cats', conditions: filter,
  ...
  Mutation: {
    updateLikeCat: async (_, args, { app, current_user }) => {
      if (!current_user)
        throw new Error(`Only logged in user can ${args.like ? 'like' : 'unlike'} a cat`);
      ...
      if (cat.user_id.toString() === current_user.id.toString())
        throw new Error(`You cannot ${args.like ? 'like' : 'unlike'} your own cat`);
    ...
    toggleStatusCat: (_, args, { app, current_user }) => {
      ...
      return helpers.toggleStatus(app.knex, {
        tableName: 'cats', idField: 'id',
        id: args.catId, status: args.active,
        entityName: 'Cat'
      })
    },
  },

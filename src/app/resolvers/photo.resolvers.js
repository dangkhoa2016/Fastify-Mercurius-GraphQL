...
module.exports = {
	Query: {
		...
		photo: async (_, { id }, context) => {
			...
			const photo = await context.app.knex('photos').where({ id }).first();
			...
			if (!photo.status) {
				const user = await context.app
					...
					.where('cats.id', photo.cat_id)
					.first('users.*');
				if (
					!current_user ||
					(current_user.role !== 'admin' &&
						user &&
						user.id.toString() !== current_user.id.toString())
				)
		...
		photosByCatId: async (
			...
			const is_current_user_request =
				current_user && current_user.id.toString() === cat.user_id.toString();
			if (
				!current_user ||
		...
		photosByUserId: async (
			...
			if (Object.keys(filter_cats).length) query = query.where(filter_cats);
			const cat_ids = await query;
			filter_photos.cat_id = cat_ids.map(({ id }) => id);

			return helpers.getWithPagination(knex, {
				tableName: 'photos',
				conditions: filter_photos,
				options: { pageIndex, pageSize },
			});
		},
	},
	...
	Mutation: {
		updateLikePhoto: async (_, args, { app, current_user }) => {
			if (!current_user)
				throw new Error(
					`Only logged in user can ${args.like ? 'like' : 'unlike'} a photo`
				);
			...
			if (cat.user_id.toString() === current_user.id.toString())
				throw new Error(
					`You can't ${args.like ? 'like' : 'unlike'} your own photo`
				);

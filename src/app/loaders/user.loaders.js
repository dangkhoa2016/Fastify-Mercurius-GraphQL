	...
	User: {
		cats_count: async (users, { app, current_user }) => {
			...
			const is_current_user_request =
				current_user &&
				users.length === 1 &&
				current_user.id.toString() === users[0].obj.id.toString();
			if (
				!current_user ||
		...
		photos_count: async (users, { app, current_user }) => {
			...
			if (Object.keys(filter_cats).length > 0) query = query.where(filter_cats);
		...
		followed: async (users, { app, current_user }) => {
			...
			const query = knex('users_followers')
				.select('user_id')
				.where('follower_id', current_user.id)
				.whereIn('user_id', user_ids);
			let followers = await query;

			followers = user_ids.map((id) =>
				followers.findIndex((follower) => follower.user_id === id) !== -1
					? true
					: false
			);
		...
		country: async (users, { app }) => {
			...
			const mapping = await knex('users')

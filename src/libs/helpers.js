	...
	try {
		...
		if (conditions && Object.keys(conditions).length) {
			for (const [key, value] of Object.entries(conditions)) {
				if (Array.isArray(value)) query = query.whereIn(key, value);
		...
		const [records, countData] = await Promise.all([
			...
			query.count('id as countName').first(),
		]);
...
const getFields = (typeName, schema) => {
	...
	return dataType instanceof GraphQLObjectType
		? Object.keys(dataType.getFields())
		: [];
};
...
const countRowsForTables = async (knex, tableNames, isAdmin = false) => {
	try {
		const rowCountPromises = tableNames.map((tableName) =>
			countRowsForTable(knex, tableName, isAdmin)
		);
...
const fetchGroupedByField = async (
	knex,
	{ isAdmin, tableName, joinTable, referenceKey, joinKey }
) => {
	...
	try {
		...
		console.error(
			`Error fetching grouped by ${joinKey}s for ${tableName}:`,
			error
		);
		throw error;
	}
...
const fetchOwners = async (
	knex,
	{ tableName, idField, relatedTableName, records, selectFields }
) => {
	...
	try {
		let query = knex(tableName)
			...
			.leftJoin(
				relatedTableName,
				`${tableName}.id`,
				`${relatedTableName}.${foreignKeyName}`
			)
			.whereIn(`${tableName}.id`, ids);

		if (filter && Object.keys(filter).length) query = query.where(filter);

		const counts = await query.groupBy(
			groupFieldName,
			`${relatedTableName}.${foreignKeyName}`
		);
		...
		return items.map(({ obj }) => {
			const count = counts.find(
				({ [groupFieldName]: itemId }) => itemId === obj.id
			);
...
const toggleStatus = async (
	knex,
	{ tableName, idField, id, status, entityName }
) => {

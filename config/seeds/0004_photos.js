...
exports.seed = async function (knex) {
  ...
  return knex('photos').del().finally(async () => {
    // create random photos for each cat
    for (const cat_id of cat_ids) {
      const number_of_photos = random(0, 100);
      if (number_of_photos > 0) {
        const photos = Array(number_of_photos).fill(null).map(() => ({
          title: random(1, 100) > 60 ? faker.location.street() : (random(1, 100) > 50 ? faker.vehicle.vehicle() : faker.airline.airport().name),

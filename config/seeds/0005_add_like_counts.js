...
async function updateLikeCountForUsers(knex) {
  ...
  for (const user_id of user_ids) {
    const cat_ids = (await knex.select('id').from('cats').where({ user_id })).map(row => row.id);
    const countCatLikes = await knex.sum('likes_count as sum_likes_count').from('cats').whereIn('id', cat_ids);
    const countPhotoLikes = await knex.sum('likes_count as sum_likes_count').from('photos').whereIn('cat_id', cat_ids);

    const likes_count = countCatLikes[0].sum_likes_count + countPhotoLikes[0].sum_likes_count;
